...
class GFFormDisplay {
	...
	public static function process_form( $form_id ) {
		...
		if ( ( $is_valid && $page_number == 0 ) || $saving_for_later ) {
			...
			if ( $failed_honeypot ) {
				...
				GFCommon::log_debug( 'Saved incomplete submission' );
	...
	public static function clean_up_files( $form ) {
		...
		// clean up files from abandoned submissions older than 48 hours (31 days if Save and Continue is enabled)
		$files = glob( $target_path . '*' );
		if ( is_array( $files ) ) {
			$seconds_in_day = 24 * 60 * 60;
			$lifespan       = rgars( $form, 'save/enabled' ) ? 31 * $seconds_in_day : 2 * $seconds_in_day;
				...
				if ( self::failed_state_validation( $form['id'], $field, $value ) ) {
					...
					switch ( $input_type ) {
						...
						case 'hiddenproduct' :
							...
							if ( $field->isRequired && rgblank( $quantity ) && ! $field->disableQuantity ) {
								$field->failed_validation  = true;
								$field->validation_message = rgempty( 'errorMessage', $field ) ? __( 'This field is required.', 'gravityforms' ) : $field->errorMessage;
