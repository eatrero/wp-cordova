...
class GF_Field_Address extends GF_Field {
	...
	function validate( $value, $form ) {
		...
		if ( $this->isRequired ) {
			$street  = $_POST[ 'input_' . $this->id . '_1' ];
			$city    = $_POST[ 'input_' . $this->id . '_3' ];
			$state   = $_POST[ 'input_' . $this->id . '_4' ];
			$zip     = $_POST[ 'input_' . $this->id . '_5' ];
			$country = $_POST[ 'input_' . $this->id . '_6' ];
			if ( empty( $street ) || empty( $city ) || empty( $zip ) || ( empty( $state ) && ! $this->hideState ) || ( empty( $country ) && ! $this->hideCountry ) ) {
	...
	public function get_field_input( $form, $value = '', $entry = null ) {
		...
		if ( $address_display_format == 'zip_before_city' ) {
			...
			if ( IS_ADMIN || ! rgar( $address_city_field_input, 'isHidden' ) ) {
				if ( $is_sub_label_above ) {
					$city = "<span class='ginput_{$city_location}{$class_suffix}' id='{$field_id}_3_container' {$style}>
                                    <label for='{$field_id}_3' id='$field_id.3_label' {$sub_label_class_attribute}>{$address_city_sub_label}</label>
                                    <input type='text' name='input_{$id}.3' id='{$field_id}_3' value='{$city_value}' {$tabindex} {$disabled_text} {$city_placeholder_attribute}/>
                                 </span>";
				} else {
					$city = "<span class='ginput_{$city_location}{$class_suffix}' id='{$field_id}_3_container' {$style}>
	...
	// testing the form editor inline script
	public function get_form_editor_inline_script_on_page_render() {
		return "console.log('testing the inline script on page render from GF_Field_Address');";
	}

	// testing the form inline script
