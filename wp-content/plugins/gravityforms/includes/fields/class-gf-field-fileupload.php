...
class GF_Field_FileUpload extends GF_Field {
		...
		if ( $is_entry_detail && ! empty( $value ) ) { // edit entry
			...
			foreach ( $file_urls as $file_index => $file_url ) {
				...
				$preview .= "<div id='preview_file_{$file_index}' class='ginput_preview'>
								...
								<img src='{$download_button_url}' style='margin-left:10px;'/></a><a href='javascript:void(0);' alt='{$delete_file_text}' title='{$delete_file_text}' onclick='DeleteFile({$lead_id},{$id},this);' >
								<img src='{$delete_button_url}' style='margin-left:10px;'/></a>
							</div>";
			}
			...
			if ( ! empty( $file_infos ) ) {
				...
				foreach ( $file_infos as $file_info ) {
					$preview .= "<div class='ginput_preview'>
									<img class='gform_delete' src='" . GFCommon::get_base_url() . "/images/delete.png' onclick='gformDeleteUploadedFile({$form_id}, {$id}, this);' /> <strong>" . esc_html( $file_info['uploaded_filename'] ) . '</strong>
								</div>';
				}
				$preview .= '</div>';
				if ( ! $multiple_files ) {
	...
	public function get_value_save_entry( $value, $form, $input_name, $lead_id, $lead ) {
		...
		if ( $this->multipleFiles ) {
			...
			$value = $this->get_fileupload_value( $form_id, $input_name );
		}

		return $value;
	}

	public function get_fileupload_value( $form_id, $input_name ) {
		global $_gf_uploaded_files;

		GFCommon::log_debug( 'Starting GF_Field_Fileupload::get_fileupload_value()' );

		if ( empty( $_gf_uploaded_files ) ) {
			GFCommon::log_debug( 'No files uploaded. Exiting GF_Field_Fileupload::get_fileupload_value()' );
