...
class GFEntryDetail {
	...
	public static function lead_detail_page() {
		...
		<h2 class="gf_admin_page_title">
			<span><?php echo __( 'Entry #', 'gravityforms' ) . absint( $lead['id'] ); ?></span><span class="gf_admin_page_subtitle"><span class="gf_admin_page_formid">ID: <?php echo $form['id']; ?></span><?php echo $form['title'];
				$gf_entry_locking = new GFEntryLocking();
				$gf_entry_locking->lock_info( $lead_id ); ?></span></h2>
				...
				<div id="submitcomment" class="submitbox">
					<div id="minor-publishing" style="padding:10px;">
						...
						if ( ! empty( $lead['created_by'] ) && $usermeta = get_userdata( $lead['created_by'] ) ) {
							?>
							<?php _e( 'User', 'gravityforms' ); ?>:
							<a href="user-edit.php?user_id=<?php echo absint( $lead['created_by'] ) ?>" alt="<?php _e( 'View user profile', 'gravityforms' ); ?>" title="<?php _e( 'View user profile', 'gravityforms' ); ?>"><?php echo esc_html( $usermeta->user_login ) ?></a>
							<br /><br />
						<?php
						}
						?>

						<?php _e( 'Embed Url', 'gravityforms' ); ?>:
						<a href="<?php echo esc_url( $lead['source_url'] ) ?>" target="_blank" alt="<?php echo esc_url( $lead['source_url'] ) ?>" title="<?php echo esc_url( $lead['source_url'] ) ?>">.../<?php echo esc_html( GFCommon::truncate_url( $lead['source_url'] ) ) ?></a>
						<br /><br />
						<?php
						if ( ! empty( $lead['post_id'] ) ) {
							$post = get_post( $lead['post_id'] );
							?>
							<?php _e( 'Edit Post', 'gravityforms' ); ?>:
							<a href="post.php?action=edit&post=<?php echo absint( $post->ID ) ?>" alt="<?php _e( 'Click to edit post', 'gravityforms' ); ?>" title="<?php _e( 'Click to edit post', 'gravityforms' ); ?>"><?php echo esc_html( $post->post_title ) ?></a>
							<br /><br />
						<?php
						}
						...
						if ( do_action( 'gform_enable_entry_info_payment_details', true, $lead ) ) {
							...
							if ( ! empty( $lead['payment_status'] ) ) {
								...
								if ( ! empty( $lead['payment_date'] ) ) {
									echo $lead['transaction_type'] != 2 ? __( 'Payment Date', 'gravityforms' ) : __( 'Start Date', 'gravityforms' ) ?>: <?php echo GFCommon::format_date( $lead['payment_date'], false, 'Y/m/d', $lead['transaction_type'] != 2 ) ?>
									<br /><br />
								<?php
								}

								if ( ! empty( $lead['transaction_id'] ) ) {
									echo $lead['transaction_type'] != 2 ? __( 'Transaction Id', 'gravityforms' ) : __( 'Subscriber Id', 'gravityforms' ); ?>: <?php echo $lead['transaction_id'] ?>
									<br /><br />
								<?php
								}

								if ( ! rgblank( $lead['payment_amount'] ) ) {
									echo $lead['transaction_type'] != 2 ? __( 'Payment Amount', 'gravityforms' ) : __( 'Subscription Amount', 'gravityforms' ); ?>: <?php echo GFCommon::to_money( $lead['payment_amount'], $lead['currency'] ) ?>
									<br /><br />
								<?php
								}
							}
