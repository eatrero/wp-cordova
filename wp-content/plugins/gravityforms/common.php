...
class GFCommon {
	...
	public static function send_notification( $notification, $form, $lead ) {
		...
		GFCommon::log_debug( "send_notification(): Preparing {$notification['id']}. Notification named '{$notification['name']}'." );
	...
	public static function send_notifications( $notification_ids, $form, $lead, $do_conditional_logic = true, $event = 'form_submission' ) {
		if ( ! is_array( $notification_ids ) || empty( $notification_ids ) ) {
			GFCommon::log_debug( "send_notifications(): No notifications to process for {$event} event." );

			return;
		}

		GFCommon::log_debug( "send_notifications(): Processing notifications for {$event} event: " . print_r( $notification_ids, true ) . "\n(only active/applicable notifications are sent)" );
		...
		foreach ( $notification_ids as $notification_id ) {
			...
			if ( isset( $form['notifications'][ $notification_id ]['isActive'] ) && ! $form['notifications'][ $notification_id ]['isActive'] ) {
				GFCommon::log_debug( "send_notifications(): Not sending {$notification_id}. The notification named '{$form['notifications'][$notification_id]['name']}' is inactive." );
				continue;
			}
			...
			if ( $do_conditional_logic && ! GFCommon::evaluate_conditional_logic( rgar( $notification, 'conditionalLogic' ), $form, $lead ) ) {
				GFCommon::log_debug( "send_notifications(): Not sending {$notification_id}. The notification named '{$notification['name']}' failed conditional logic checks." );
				continue;
			}
	...
	private static function send_email( $from, $to, $bcc, $reply_to, $subject, $message, $from_name = '', $message_format = 'html', $attachments = '' ) {
		...
		$is_success = false;
		if ( ! $abort_email ) {
			GFCommon::log_debug( 'send_email(): Sending email via wp_mail()' );
			...
			GFCommon::log_debug( "send_email(): Result from wp_mail(): {$is_success}" );
			if ( $is_success ) {
				GFCommon::log_debug( 'send_email(): Mail was passed from WordPress to the mail server.' );
			} else {
				GFCommon::log_debug( 'send_email(): The mail message was passed off to WordPress for processing, but WordPress was unable to send the message.' );
			}
		}
	...
	public static function get_field_placeholder_attribute( $field ) {
		...
		return ! empty( $placeholder_value ) ? sprintf( "placeholder='%s'", $placeholder_value ) : '';
	}
	...
	public static function get_field_type_title( $type ) {
		$gf_field = GF_Fields::get_instance( $type );
		if ( ! empty( $gf_field ) ) {
	...
	public static function get_us_state_code( $state_name ) {
		_deprecated_function( 'GFCommon::get_us_state_code', '1.9', 'GF_Field_Address::get_us_state_code' );

		return GF_Fields::get_instance( 'address' )->get_us_state_code( $state_name );
	}

	public static function get_country_code( $country_name ) {
		_deprecated_function( 'GFCommon::get_country_code', '1.9', 'GF_Field_Address::get_country_code' );

		return GF_Fields::get_instance( 'address' )->get_country_code( $country_name );
	}
	...
	public static function gf_vars( $echo = true ) {
		...
		$gf_vars['customChoices']     = __( 'Custom Choices', 'gravityforms' );
		$gf_vars['predefinedChoices'] = __( 'Predefined Choices', 'gravityforms' );
